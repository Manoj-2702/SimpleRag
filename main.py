...
def initialize_database(list_file_obj, progress=gr.Progress()):
    ...
    list_file_path = upload_file(list_file_obj)
...
def upload_file(file_paths):
    ...
    if isinstance(file_paths, list):
        ...
        list_file_path.append(file_paths)
    return list_file_path


def create_dataset(pdf_path, vector_db):
    doc_splits = load_doc([pdf_path])
    embeddings = HuggingFaceEmbeddings()
    vectordb = FAISS.from_documents(doc_splits, embeddings)
    
    query_response_pairs = []
    # Assuming the document has at least 30 chunks
    for i, doc in enumerate(doc_splits[:30]):
        query = f"What is discussed in chunk {i+1}?"
        response = vector_db.similarity_search(query, k=1)
        query_response_pairs.append((query, response[0].page_content))
    
    df = pd.DataFrame(query_response_pairs, columns=["Query", "Response"])
    df.to_csv("query_response_pairs.csv", index=False)
    return df


def evaluate_model(dataset_path, qa_chain):
    ...
    for index, row in dataset.iterrows():
        ...
        predicted_response = response["answer"]
        
        if predicted_response.strip().lower() == true_response.strip().lower():
...
def demo():
    with gr.Blocks(theme=gr.themes.Default(primary_hue="sky")) as demo:
        vector_db=gr.State()
        qa_chain=gr.State()
        gr.HTML("<center><h1>RAG Model for Athina AI</h1></center>")
        gr.Markdown("""<b>Query your PDF Doc. </b>
                This AI Agent is designed to perform retrieval augmented generation (RAG) on PDF documents. 
                The app is hosted on Hugging Face Hub for the sole purpose of demonstration. 
                <b> Its at your own risk to upload Confidential Documents.</b>
                    """)
